...
double CodeTable::compute_szD_CT(DittoSequence *sequence)
{
	...
	for (int s = 0; s < sequence->get_nrSequences(); ++s)								//TERM: sum_{j in s(D)} L_N(|S_j|)
	...
	codeTableSet::iterator it_ct = g_codeTable->begin(), it_end = g_codeTable->end();
	for(it_ct; it_ct != it_end ; ++it_ct)	
	{
		//skip fill-patterns
		if( par->fillDittoPatterns && (*it_ct)->getSize() == 1 && par->fillDittoPattern[(*(*it_ct)->getSymbols(0)->begin())->attribute] && (*(*it_ct)->getSymbols(0)->begin())->symbol == par->alphabetSizes[(*(*it_ct)->getSymbols(0)->begin())->attribute]-1 )
			continue;
		...
		if ((*it_ct)->getUsage() > 0)
		{
			...
			{
				size += (*it_ct)->getUsageGap() * (*it_ct)->getCodelengthGap();			//TERM: L(C_g | CT)		-> gap codes
				size += (*it_ct)->getUsageFill() * (*it_ct)->getCodelengthFill();		//TERM: L(C_g | CT)		-> no_gap codes
			}
		}
	}
	...
	if(sequence->getInputType() == CATEGORICAL)
	{
		for (int aid = 0; aid < nrAttr; ++aid)
		{
			...
			if(par->fillDittoPatterns && par->fillDittoPattern[aid])
				alph -= 1;					   
			size += mu->intcost(alph);													//TERM: L_N(|Omega_aid|)					-> #singletons 
			size += mu->lg_choose(par->nrMulti_events, alph);							//TERM: log( ||D^aid|| OVER |Omega_aid| )	-> singleton supports 
		}
	}
	else //ITEMSET 
		size += mu->lg_choose(sequence->get_nrEvents(), par->nrOfAttributes);			//TERM: log(||D|| OVER |A|)					-> support for all attributes; divide all events over all singletons. NOTE: singletons can have zero support -> thus NOT log(||D||-1 OVER |A|-1)

	int non_singletons = length_CT;	
	for (int aid = 0; aid < nrAttr; ++aid) 
		non_singletons -= alphabetSizes[aid];
	int non_singl_usg = 0;	
	codeTableSet::iterator it_ct = g_codeTable->begin(), it_end = g_codeTable->end();	//TERM: L(X in CT) for all non-singleton patters
	for (it_ct; it_ct != it_end; ++it_ct)
	{
		...
		size += mu->intcost(p->getLength());											//TERM: L_N( |X| )							-> length of the pattern 
			
		for (int ts = 0; ts < p->getLength(); ++ts)										
			size += logb2(nrAttr) ;														//TERM: log( |A| )							-> height of the pattern at ts 

		size += mu->intcost(p->getUsageGap() + 1);										//TERM: L_N( gaps(X)+1 )					-> #gaps in X 
		size += p->getSTsize();															//TERM: SUM_{x in X} L( code_p(x|ST) )		-> length of X in left column of CT
	}
	
	size += mu->intcost(non_singletons + 1);											//TERM: L_N( |P|+1 )						-> #non-singletons 

	size += mu->intcost(non_singl_usg + 1);												//TERM: L_N( usage(P)+1 )					-> total usage of non-singletons 

	if(non_singl_usg != 0)
	{
		if(non_singl_usg < non_singletons)												//one non-singletons with usage=0, rest with usage=1
			size += logb2(non_singletons);												//indicate the one with usage=0
		else
			size += mu->lg_choose(non_singl_usg, non_singletons);						//TERM: log( usage(P) OVER |P|)				-> the usage per non-singleton pattern 
	}
...
*/
const double CodeTable::compute_sz(DittoSequence *sequence)
{
	double size = 0;											
	size += compute_szD_CT(sequence);				//L(D | CT)
	
	double temp = compute_szCT_C(sequence);
	if(szCT_C == 0)									//when the final CT has been computed we can reuse this size because it does not change when encoding other data
		size += temp;			//L(CT | C)
	else
		size += szCT_C;
...
void CodeTable::deleteDittoPattern(DittoPattern* p)
{
	...
	codeTableSet::iterator fnd = g_codeTable->find(p), end = g_codeTable->end();
	if(fnd != end)
	{
...
string CodeTable::printcandidateSet(bool consoleOutput, candpatternSet *plist, string title) const
{
	stringstream result;
...
string CodeTable::printpatternSet(bool consoleOutput, codeTableSet *plist, string title) const
{
...
{
	list<usgSz>	*result = new list<usgSz>;
	codeTableSet::iterator it = g_codeTable->begin(), end = g_codeTable->end();
	int cnt = 0;
	int szOld = (*it)->getSize();
	while(it != end)
	{
