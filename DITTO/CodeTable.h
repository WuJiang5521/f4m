...
{
	public:
		CodeTable(DittoSequence *s);
		~CodeTable(void);

		double			compute_szD_CT(DittoSequence *s);
		double			compute_szCT_C(DittoSequence *s);
		const double	compute_sz(DittoSequence *s);							//used when we use the CodeTable to compress other data
		...
		string			printpatternSet(bool consoleOutput, codeTableSet *plist, string title) const;
		string			printcandidateSet(bool consoleOutput, candpatternSet *plist, string title) const;
		...
		void			set_szCT_C(DittoSequence *s) { szCT_C = compute_szCT_C(s); }; //only called when the final CT for this data is computed
