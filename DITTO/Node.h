...
class Node {
public:
    Node(int alphabetSize, int *alphabetSizes, int timestep) : g_alphabetSize(alphabetSize),
                                                               g_alphabetSizes(alphabetSizes), g_timestep(timestep) {
        g_tree_id = 0;    //ROOT
        init(-1);
    }

    Node(int alphabetSize, int *alphabetSizes, int timestep, int id, int aid) : g_alphabetSize(alphabetSize),
                                                                                g_alphabetSizes(alphabetSizes),
    ...
    bool findDittoPattern(DittoPattern *p, int startPos, eventSet::iterator it,
                          eventSet::iterator end)        //True when eventSet found and thus not frequent, False otherwise
    {
        if (g_infrequent)
            return true;
        ...
        if (it == end)        //check if there is a next node to go to
        {
            if (g_timestep == p->getLength() - 1 - startPos)
                ...
                if (g_next_timestep == nullptr)
                    ...
                    return g_next_timestep->findDittoPattern(p, startPos,
                                                             p->getSymbols(startPos + g_timestep + 1)->begin(),
                                                             p->getSymbols(startPos + g_timestep + 1)->end());
            }
        }
    ...
    void addInfrequentDittoPattern(DittoPattern *p, eventSet::iterator it,
                                   eventSet::iterator end)    //evs is SORTED: first ascending on attribute-level and then ascending alphabetically
    {
        if (it == end) {
            if (g_timestep == p->getLength() - 1)
                ...
                g_next_timestep->addInfrequentDittoPattern(p, p->getSymbols(g_timestep + 1)->begin(),
                                                           p->getSymbols(g_timestep + 1)->end());
            }
        } else {
