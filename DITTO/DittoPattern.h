...
public:
	...
	list<Window*>* getMinWindows(DittoSequence *seq, bool otherData) { if(!otherData) return minWindows; else if(altMinWindows == 0) altMinWindows = buildMinWindows(seq); return altMinWindows;   }
	bool getUsageDecreased() { return usageDecreased; }
	int getUsage() const { return usage; }
	int getEstimatedUsage() { if(g_x == 0) return usage; else return min(g_x->getUsage(), g_y->getUsage()); }	
	int getUsageGap() const { return usageGap; }
	int getUsageFill() const { return usageFill; }
	double getCodelength() { return codelength; }
	double getCodelengthGap() { return codelengthGap; }
	double getCodelengthFill() { return codelengthFill; }
	...
	void updateUsages(int usgGap) { usage++; usageGap += usgGap; usageFill += length-1; } 
	...
	void loadWindowsAndSupport(DittoPattern *p) { support = p->getSupport(); minWindows = p->getMinWindows(g_seq, false); }					//only used for g_blackList/whitelist

	void set_info(string s) { g_info += s; }
	void setMinWindows(DittoSequence *s) { minWindows = buildMinWindows(s); }
	void setSTsize(double** baseCodeLengths);
	void resetUsage();
...
*/
	...
	{
		//ascending on length
		if (ts > lhs.getLength() -1)				//up to here everything equal, but rhs is longer than lhs, thus lhs has higher order
			return true;
		if (ts > rhs.getLength() -1)
			return false;

		eventSet *avts = lhs.getSymbols(ts), *bvts = rhs.getSymbols(ts);
		eventSet::iterator ita = avts->begin(), enda = avts->end(), itb = bvts->begin(), endb = bvts->end();
		while(ita != enda && itb != endb)
		{
			...
			++ita;
			++itb;
			if(ita == enda && itb != endb)		//lhs is smaller on this time step but all its events are similar to rhs
				return true;
...
struct dummy
{
	dummy(int size, int length, attrSymSet** events, int support, float gapChance) : size(size), length(length), events(events), support(support), gapChance(gapChance)
	{	
	}
	
	bool containsAttrSym(int ts, int aid, int sym)
	{
		attrSymSet::iterator it = events[ts]->begin(), end = events[ts]->end();
		while(it != end)
		{
	...
	string toString() { stringstream ss; ss << "sz: " << size << "\tlen: " << length; for(int l = 0; l < length; ++l){ ss << "{"; attrSymSet::iterator it = events[l]->begin(), end = events[l]->end(); while(it != end) { ss << (*it)->aid << "." << (*it)->sym; ++it; if(it != end) ss << ",";} ss << "}";} ss << "\tsup: " << support << "\tgapChance: " << gapChance; return ss.str();  }
...
struct Parameters
{
	...
	string	toString() 
	{
		...
		ss << date << "; " << eind-start  << "; " << STsize << "; " << CTsize << "; " << perc << "; " << lengthCT << "; " << nr_non_singletons << "(";
		list<usgSz>::iterator it = nr_non_singletons_per_size->begin(), end = nr_non_singletons_per_size->end();
		while(it != end)
		{
			ss << (*it).sz << "-" << (*it).usg << " ";
			++it;
		...
		ss << cntMatPat << "; " << cntCovers << "; " << cntAcc << "; " << cntRej << "; " << cntAccVar << "; " << cntRejVar << "; " << cntInfreqMaterialized << "; " << cntInfreq << "; " << inputFilename << "; " << outputFilename << "; " << gapvariants << "; " << blacklist << "; " << whitelist << "; " <<pruneEstGain << "; " <<prune_tree << "; " << input_type << "\n";
		return ss.str();
	}
	...
	string	print() 
	{
		...
		ss << ")\n" << "Breakdown of found patterns, hidden: " << nrOfDittoPatterns << " total found: " << nr_non_singletons << " Exact: " <<  cntExact << " Subset: " << cntSubset << " Union_subset: " << cntUnion_subset << " unrelated: " << cntUnrelated << endl;
		ss << "cntMatPat: " << cntMatPat << "  cntCovers: " << cntCovers << "  cntAcc: " << cntAcc << "  cntRej: " << cntRej << "  cntAccVar: " << cntAccVar << "  cntRejVar: " << cntRejVar << "  cntInfreqMat: " << cntInfreqMaterialized << "  cntInfreq: " << cntInfreq << "\n";
		return ss.str();
	}
