...
void DittoSequence::computeSTCodelengths() {
    ...
    for (int aid = 0; aid < par->nrOfAttributes; ++aid) {
        ...
        for (int s = 0; s < par->alphabetSizes[aid]; ++s)
            g_STcodelengths[aid][s] = -lg2(
                    (g_occ[aid][s].size() + laplace) / (g_nrEvents + par->alphabetSize * laplace));
    }
...
const list<Multi_event *> *DittoSequence::findOccurrences(eventSet *events) const {
    ...
    auto *occPerEvent = new list<Multi_event *>::iterator[nrEvents];        //for each event an iterator to the list of Multi_event where it occurs
    auto *endPerEvent = new list<Multi_event *>::iterator[nrEvents];        //for each event an iterator to the end of the list of Multi_event where it occurs
    int i = 0;
    for (auto event : *events) {
        occPerEvent[i] = g_occ[event->attribute][event->symbol].begin();
        endPerEvent[i] = g_occ[event->attribute][event->symbol].end();
        ++i;
    }
    ...
    while (true) {
        ...
        aligned = true;
        for (int j = 1; j < nrEvents; ++j) {
            if (occPerEvent[j] == endPerEvent[j])
                break;

            if ((*occPerEvent[j])->id <
                smallestID)                    //keep track of the event with the earliest occurence
            {
                smallestID = (*occPerEvent[j])->id;
                smallest = j;
            }
    ...
    for (int ts = 0; ts < p->getLength(); ++ts) {
        ...
        for (auto event : *events) {
            if (result) {
                ...
                g_outputStream << "The next event is: ";
                event->print();
                g_outputStream << "The pattern p: ";
                p->print();
                g_outputStream << "The window w: ";
                w->print();
                g_outputStream << "\n\n\n\n\n";
            }
...
void DittoSequence::coverSingletons(DittoPattern ***singletons) {
    for (int i = 0; i < par->nrMulti_events; ++i) {
        ...
        for (auto it = g_mev_time[i]->getEvents()->begin(); it != g_mev_time[i]->getEvents()->end(); ++it) {
...
string DittoSequence::print_sequence(bool allValues) {
    ...
    g_outputStream << "SEQUENCE. Alphabet size: " << par->alphabetSize << " #Attributes: " << par->nrOfAttributes
                   << " alphabet sizes: {";
    for (int i = 0; i < par->nrOfAttributes; ++i) {
    ...
    {
        for (int i = 0; i < par->nrMulti_events; ++i) {
            for (auto it: *(g_mev_time[i]->getEvents())) {
                g_outputStream << i << ":  ";
                it->print();
            }
            g_outputStream << endl;
